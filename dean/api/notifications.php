<?php
error_log("Dean Notifications.php accessed with action: " . (isset($_GET['action']) ? $_GET['action'] : 'none'));
session_start();
...
// Add debugging
error_log("Dean session data: " . print_r($_SESSION, true));

if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'dean') {
    error_log("No dean user_id in session");
    echo json_encode(['success' => false, 'message' => 'Not authenticated as dean', 'session' => $_SESSION]);
    exit();
}
...
switch ($action) {
    case 'get_notifications':
        ...
        foreach ($notifications as &$notification) {
            $notification['url'] = getDeanNotificationUrl($notification);
        }
