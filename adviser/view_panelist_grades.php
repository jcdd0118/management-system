...
$pdfField = $defenseType === 'final' ? 'final_defense_pdf' : 'pdf_file';

$query = "
    SELECT d.*, pw.project_title, pw.proponent_1, pw.proponent_2, pw.proponent_3, pw.proponent_4, s.group_code
    FROM $defenseTable d
    INNER JOIN project_working_titles pw ON d.project_id = pw.id
...
// Check if the adviser is authorized to view this project
$projectId = $defense['project_id'];
$adviserCheckQuery = "SELECT noted_by FROM project_working_titles WHERE id = ?";
$adviserCheckStmt = $conn->prepare($adviserCheckQuery);
$adviserCheckStmt->bind_param("i", $projectId);
$adviserCheckStmt->execute();
$adviserCheckResult = $adviserCheckStmt->get_result();
$projectData = $adviserCheckResult->fetch_assoc();
$adviserCheckStmt->close();

// Debug: Let's see what we're comparing
if (!$projectData) {
    header("Location: " . ($defenseType === 'final' ? 'final_defense_list.php' : 'title_defense_list.php') . "?error=project_not_found");
    exit();
}

// Convert both to integers for comparison
$notedBy = (int)$projectData['noted_by'];
$adviserId = (int)$adviserId;

if ($notedBy !== $adviserId) {
    // For multi-role users, let's also check if they have adviser role
    $roleCheckQuery = "SELECT roles FROM users WHERE id = ?";
    $roleCheckStmt = $conn->prepare($roleCheckQuery);
    $roleCheckStmt->bind_param("i", $adviserId);
    $roleCheckStmt->execute();
    $roleCheckResult = $roleCheckStmt->get_result();
    $userData = $roleCheckResult->fetch_assoc();
    $roleCheckStmt->close();
    
    if (!$userData || !hasRole($userData, 'adviser')) {
        header("Location: " . ($defenseType === 'final' ? 'final_defense_list.php' : 'title_defense_list.php') . "?error=unauthorized");
        exit();
    }
}

// Always resolve the group code from the defense submitter (most reliable)
$gcTable = ($defenseType === 'final') ? 'final_defense' : 'title_defense';
$gcSql = "
    SELECT COALESCE(s.group_code, '') AS group_code
    FROM $gcTable d
    INNER JOIN users u ON d.submitted_by = u.id
    LEFT JOIN students s ON u.id = s.user_id
    WHERE d.id = ?
";
$groupCode = '';
$gcStmt = $conn->prepare($gcSql);
if ($gcStmt) {
    $gcStmt->bind_param("i", $defenseId);
    $gcStmt->execute();
    $gcRes = $gcStmt->get_result();
    if ($gcRow = $gcRes->fetch_assoc()) {
        $groupCode = $gcRow['group_code'];
    }
    $gcStmt->close();
}
// Fallback to value from initial join if still empty
if ($groupCode === '' && isset($defense['group_code'])) {
    $groupCode = $defense['group_code'];
}
    ...
    <div class="container">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-8">
                        <p><strong>Project Title:</strong> <?php echo htmlspecialchars($defense['project_title']); ?></p>
                        <p><strong>Group Code:</strong> <?php echo htmlspecialchars($groupCode); ?></p>
                        <p><strong>Project Proponents:</strong></p>
                        <ul>
                            <?php if (!empty($defense['proponent_1'])): ?>
                                <li><?php echo htmlspecialchars($defense['proponent_1']); ?></li>
                            <?php endif; ?>
                            <?php if (!empty($defense['proponent_2'])): ?>
                                <li><?php echo htmlspecialchars($defense['proponent_2']); ?></li>
                            <?php endif; ?>
                            <?php if (!empty($defense['proponent_3'])): ?>
                                <li><?php echo htmlspecialchars($defense['proponent_3']); ?></li>
                            <?php endif; ?>
                            <?php if (!empty($defense['proponent_4'])): ?>
                                <li><?php echo htmlspecialchars($defense['proponent_4']); ?></li>
                            <?php endif; ?>
                        </ul>
                    </div>
                    <div class="col-md-4">
                        <p><strong>Uploaded File:</strong><br>
                        <a href="<?php echo htmlspecialchars($defense[$pdfField]); ?>" target="_blank" class="btn btn-outline-primary btn-sm">
                            ...
                            <tbody>
                                <?php foreach ($panelists as $panelist): ?>
                                    <tr>
                                        ...
                                        <?php foreach ($students as $student): ?>
                                            <td class="text-center">
                                                <?php if (isset($panelistGrades[$panelist['id']][$student['id']])): ?>
                                                    <div>
                                                        <span class="grade-value text-success">
                                                            <?php echo number_format($panelistGrades[$panelist['id']][$student['id']]['individual_grade'], 2); ?>
                                                        </span>
                                                    </div>
                                                    <?php if (!empty($panelistGrades[$panelist['id']][$student['id']]['remarks'])): ?>
                                                        <div class="student-name mt-1">
                                                            <small><?php echo htmlspecialchars($panelistGrades[$panelist['id']][$student['id']]['remarks']); ?></small>
                                                        </div>
                                                    <?php endif; ?>
                                                <?php else: ?>
                                                    <span class="text-muted">-</span>
