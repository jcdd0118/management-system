...
// Fetch all advisers (users with role 'adviser')
$query = "SELECT * FROM users WHERE role = 'adviser' ORDER BY id DESC";
...
<head>
    <meta charset="UTF-8">
    <title>Adviser List | Captrack Vault</title>
...
<div class="main-content">
    ...
    <!-- Adviser List -->
    <h4 class="mb-3">Adviser List</h4>

    <div class="d-flex justify-content-between align-items-center mb-3">
        <div class="d-flex flex-column flex-md-row justify-content-end">
            <button class="btn btn-primary btn-sm d-block d-md-inline w-100 w-md-auto mb-2 mb-md-0" data-bs-toggle="modal" data-bs-target="#addAdviserModal">
                <i class="bi bi-person-plus-fill"></i> Add Adviser
            </button>
        </div>
    </div>
            ...
            <tbody>
                <?php
                    if (mysqli_num_rows($result) > 0) {
                            ...
                            echo "<td class='text-center'>
                                    <div class='dropdown'>
                                        ...
                                        <ul class='dropdown-menu dropdown-menu-end'>
                                            <li>
                                                <button class='dropdown-item' onclick='viewAdviser(\"" .
                                                    addslashes($row['id']) . "\", \"" .
                        ...
                        echo "<tr><td colspan='4' class='text-center'>No advisers found.</td></tr>";
                    }
                ?>
            </tbody>
        </table>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content rounded-4 shadow-sm">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title" id="addAdviserModalLabel"><i class="bi bi-person-plus-fill"></i> Add Adviser</h5>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content shadow">
      ...
      <div class="modal-body">
        Are you sure you want to delete <strong id="adviserName"></strong>?<br>This action will also remove the user account.
      </div>
...
<!-- View Adviser Modal -->
<div class="modal fade" id="viewAdviserModal" tabindex="-1" aria-labelledby="viewAdviserModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content rounded-4">
      <div class="modal-header bg-info text-white">
        <h5 class="modal-title"><i class="bi bi-person-lines-fill"></i> View Adviser</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="viewAdviserForm" method="POST" action="update_adviser.php">
        <div class="modal-body px-4 py-3">
          <input type="hidden" name="id" id="adviserId">
          <div class="row g-3">
            <div class="col-md-4">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (errorMsg && errorDiv) {
        ...
        // Show the modal
        var addAdviserModal = new bootstrap.Modal(document.getElementById('addAdviserModal'));
        addAdviserModal.show();
...
<div class="modal fade" id="successModal" tabindex="-1" aria-labelledby="successModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-4">
      ...
      <div class="modal-body">
        Adviser information saved successfully!
      </div>
      <div class="modal-footer">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  confirmDeleteModal.addEventListener('show.bs.modal', function (event) {
    ...
    const adviserId = button.getAttribute('data-id');
    const adviserName = button.getAttribute('data-name');

    adviserNameEl.textContent = adviserName;
    confirmBtn.href = `delete_adviser.php?id=${adviserId}`;
  });
});
</script>
...
function viewAdviser(id, first, middle, last, email, gender) {
  // Reset form to clear old data and reset button states
  document.getElementById('viewAdviserForm').reset();
  ...
  // Fill inputs
  document.getElementById('adviserId').value = id;
  ...
  document.querySelectorAll('#viewAdviserForm input:not([name="email"]):not([type="hidden"])').forEach(el => {
    el.setAttribute('readonly', true);
  });

  // Disable selects
  document.querySelectorAll('#viewAdviserForm select').forEach(el => {
  ...
  // Show modal
  var modal = new bootstrap.Modal(document.getElementById('viewAdviserModal'));
  modal.show();
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const form = document.getElementById('viewAdviserForm');
  ...
  cancelBtn.addEventListener('click', () => {
    ...
    document.getElementById('adviserId').value = originalFormData.id;
