...
// Fetch all deans (users with role 'dean')
$query = "SELECT * FROM users WHERE role = 'dean' ORDER BY id DESC";
$result = mysqli_query($conn, $query);

// Count number of deans
$dean_count = mysqli_num_rows($result);
...
<head>
    <meta charset="UTF-8">
    <title>Dean | Captrack Vault</title>
...
<div class="main-content">
    ...
    <!-- Dean List -->
    <h4 class="mb-3">Dean of the College of Computer Studies</h4>

    <div class="d-flex justify-content-between align-items-center mb-3">
      <div class="d-flex flex-column flex-md-row justify-content-end">
          <?php if ($dean_count == 0): ?>
          <button class="btn btn-primary btn-sm d-block d-md-inline w-100 w-md-auto mb-2 mb-md-0" data-bs-toggle="modal" data-bs-target="#addDeanModal">
              <i class="bi bi-person-plus-fill"></i> Add Dean
          </button>
          <?php endif; ?>
      </div>
    </div>
            ...
            <tbody>
                <?php
                    if (mysqli_num_rows($result) > 0) {
                            ...
                            echo "<td class='text-center'>
                                    <div class='dropdown'>
                                        ...
                                        <ul class='dropdown-menu dropdown-menu-end'>
                                            <li>
                                                <button class='dropdown-item' onclick='viewDean(\"" .
                                                    addslashes($row['id']) . "\", \"" .
                        ...
                        echo "<tr><td colspan='4' class='text-center'>No deans found.</td></tr>";
                    }
                ?>
            </tbody>
        </table>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content rounded-4 shadow-sm">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title" id="addDeanModalLabel"><i class="bi bi-person-plus-fill"></i> Add Dean</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form action="add_dean.php" method="POST">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content shadow">
      ...
      <div class="modal-body">
        Are you sure you want to delete <strong id="deanName"></strong>?<br>This action will also remove the user account.
      </div>
...
<!-- View Dean Modal -->
<div class="modal fade" id="viewDeanModal" tabindex="-1" aria-labelledby="viewDeanModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content rounded-4">
      <div class="modal-header bg-info text-white">
        <h5 class="modal-title"><i class="bi bi-person-lines-fill"></i> View Dean</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="viewDeanForm" method="POST" action="update_dean.php">
        <div class="modal-body px-4 py-3">
          <input type="hidden" name="id" id="deanId">
          <div class="row g-3">
            <div class="col-md-4">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (errorMsg && errorDiv) {
        ...
        // Show the modal
        var addDeanModal = new bootstrap.Modal(document.getElementById('addDeanModal'));
        addDeanModal.show();
...
<div class="modal fade" id="successModal" tabindex="-1" aria-labelledby="successModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-4">
      ...
      <div class="modal-body">
        Dean information saved successfully!
      </div>
      <div class="modal-footer">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const deanNameEl = document.getElementById('deanName');
  const confirmBtn = document.getElementById('confirmDeleteBtn');

  confirmDeleteModal.addEventListener('show.bs.modal', function (event) {
    const button = event.relatedTarget;
    const deanId = button.getAttribute('data-id');
    const deanName = button.getAttribute('data-name');

    deanNameEl.textContent = deanName;
    confirmBtn.href = `delete_dean.php?id=${deanId}`;
  });
});
</script>
...
function viewDean(id, first, middle, last, email, gender) {
  // Reset form to clear old data and reset button states
  document.getElementById('viewDeanForm').reset();
  ...
  // Fill inputs
  document.getElementById('deanId').value = id;
  ...
  document.querySelectorAll('#viewDeanForm input:not([name="email"]):not([type="hidden"])').forEach(el => {
    el.setAttribute('readonly', true);
  });

  // Disable selects
  document.querySelectorAll('#viewDeanForm select').forEach(el => {
  ...
  // Show modal
  var modal = new bootstrap.Modal(document.getElementById('viewDeanModal'));
  modal.show();
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const form = document.getElementById('viewDeanForm');
  ...
  cancelBtn.addEventListener('click', () => {
    ...
    document.getElementById('deanId').value = originalFormData.id;
