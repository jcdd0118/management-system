...
function add_adviser($data, $conn) {
    ...
    if (!$stmt->execute()) {
        $stmt->close();
        return "Failed to add adviser: " . $stmt->error;
    }
    $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $result = add_adviser($_POST, $conn);

    if ($result === true) {
        $_SESSION['success_message'] = "Adviser added successfully.";
        ...
        header("Location: adviser_list.php?show_modal=true");
        exit();
    }
}
?>