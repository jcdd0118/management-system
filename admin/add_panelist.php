...
function add_panelist($data, $conn) {
    ...
    if (!$stmt->execute()) {
        $stmt->close();
        return "Failed to add panelist: " . $stmt->error;
    }
    $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $result = add_panelist($_POST, $conn);

    if ($result === true) {
        $_SESSION['success_message'] = "Panelist added successfully.";
        ...
        header("Location: panelist_list.php?show_modal=true");
        exit();
    }
}
?>