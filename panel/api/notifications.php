<?php
error_log("Panel Notifications.php accessed with action: " . (isset($_GET['action']) ? $_GET['action'] : 'none'));
session_start();
...
// Add debugging
error_log("Panel session data: " . print_r($_SESSION, true));

if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'panelist') {
    error_log("No panelist user_id in session");
    echo json_encode(['success' => false, 'message' => 'Not authenticated as panelist', 'session' => $_SESSION]);
    exit();
}
...
switch ($action) {
    case 'get_notifications':
        ...
        foreach ($notifications as &$notification) {
            $notification['url'] = getPanelistNotificationUrl($notification);
        }
