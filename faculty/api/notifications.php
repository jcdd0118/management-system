<?php
error_log("Faculty Notifications.php accessed with action: " . (isset($_GET['action']) ? $_GET['action'] : 'none'));
session_start();
...
// Add debugging
error_log("Faculty session data: " . print_r($_SESSION, true));

if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'faculty') {
    error_log("No faculty user_id in session");
    echo json_encode(['success' => false, 'message' => 'Not authenticated as faculty', 'session' => $_SESSION]);
    exit();
}
...
switch ($action) {
    case 'get_notifications':
        ...
        foreach ($notifications as &$notification) {
            $notification['url'] = getFacultyNotificationUrl($notification);
        }
