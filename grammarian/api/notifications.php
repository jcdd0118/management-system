<?php
error_log("Grammarian Notifications.php accessed with action: " . (isset($_GET['action']) ? $_GET['action'] : 'none'));
session_start();
...
// Add debugging
error_log("Grammarian session data: " . print_r($_SESSION, true));

if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'grammarian') {
    error_log("No grammarian user_id in session");
    echo json_encode(['success' => false, 'message' => 'Not authenticated as grammarian', 'session' => $_SESSION]);
    exit();
}
...
switch ($action) {
    case 'get_notifications':
        ...
        foreach ($notifications as &$notification) {
            $notification['url'] = getGrammarianNotificationUrl($notification);
        }
